document.addEventListener("DOMContentLoaded", function (event) {
  // Page scroll using url params
let url = new URL(window.location.href);
if (url.searchParams.has('sitelink')) {
  const sitelink = url.searchParams.get('sitelink');
 ...
 // Auto Enquiry Modal
  ...
  // PP Iframe
  let ppIframeURL = "https://www.propertypistol.com/aboutus/";
  $('.ppiframebtn').on('click', () => {
  	$(".pp-iframe").attr("src", ppIframeURL);
  });
  $('#ppiframe').on('hide.bs.modal', function (event) {
    $(".pp-iframe").attr("src","");
  });
